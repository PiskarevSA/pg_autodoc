...
${table['schema_dot']}.\
      % endif
${table['table_dot']}" [fontname = helvetica, shape = plaintext, label = < <TABLE BORDER="1" CELLBORDER="0" CELLSPACING="0"> \
...
${fk_link['handle1_schema']}.\
  % endif
${fk_link['handle1_name']}":ltcol${fk_link['handle1_connection']} [label=" "];
% endfor
}
